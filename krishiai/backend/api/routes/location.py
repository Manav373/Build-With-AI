"""
app/api/location.py – Farmer Location Intelligence Endpoints
-------------------------------------------------------------
...
logger = logging.getLogger("KrishiMCP.Location")
...
# ---------------------------------------------------------------------------
# Reverse geocode helper (uses OpenWeatherMap Geocoding API already configured)
# ---------------------------------------------------------------------------
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=3.0, headers={"User-Agent": "KrishiAI/1.1"}) as client:
            ...
            if r.status_code == 200:
                ...
                # Granular Indian address mapping
...
def get_farmer_locations(db: Session = Depends(get_db), user_data: dict = Depends(verify_clerk_token)):
    ...
    Lightweight — no PII, just coordinates + source.
    Returns `latitude`/`longitude` to match the frontend FarmerHeatmap component.
    """
...
def get_analytics(db: Session = Depends(get_db), user_data: dict = Depends(verify_clerk_token)):
    ...
    Returns KPI counts, state breakdown, source split, and daily active.
    """
    rows = db.query(FarmerLocation).all()
    ...
    # Farmers by Taluka (top 10) - New accurate metric
    ...
    return {
        ...
        "location_count":   total,           # alias for KPI card
...
async def re_geocode_unknown(db: Session = Depends(get_db), user_data: dict = Depends(verify_clerk_token)):
    """
    One-time admin endpoint: re-geocode all records that have state/city = 'Unknown'.
    Safe to call multiple times — only touches Unknown records.
    """
