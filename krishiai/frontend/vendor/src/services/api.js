...
export const sendChatQuery = async (phoneId, message, lat = null, lon = null, history = [], city = null, state = null, token = null, village = null, taluka = null) => {
  try {
    ...
    const payload = { 
      phone_id: phoneId, 
      message: message,
      // Send last 10 turns of history for context (trim to avoid huge payloads)
...
// 4. Send Farmer Location (for analytics)
export const sendLocation = async (latitude, longitude, source = 'web', token = null) => {
  try {
    ...
    // Silently fail — location is optional, do not break UX
  }
};

// 5. Get all farmer locations (for heatmap)
...
// 9. Resolve Coordinates or IP to District (Backend Logic)
