...
function ApplicationsModal({ isOpen, onClose, requirement }) {
      ...
      setApps([
        {
          ...
          location: `${requirement.procurement_location || 'Hadapsar'}, Pune (12 km away)`,
          quantity: `40 ${requirement.quantity_unit || 'Quintals'}`,
          price: `₹${requirement.min_price || 2400} / ${requirement.price_unit || 'quintal'}`,
          quality: 'Grade A-1 Organic, Moisture < 9.5%',
          status: 'pending',
        ...
        },
        {
          ...
          location: `Shirur, Pune (24 km away)`,
          quantity: `75 ${requirement.quantity_unit || 'Quintals'}`,
          price: `₹${Math.round(((requirement.min_price || 2200) + (requirement.max_price || 2600)) / 2)} / ${requirement.price_unit || 'quintal'}`,
          quality: 'Grade A Super, Cleaned & Sorted',
          status: 'pending',
        ...
        },
        {
          ...
          location: `Baramati, Pune (45 km away)`,
          quantity: `120 ${requirement.quantity_unit || 'Quintals'}`,
          price: `₹${requirement.max_price || 2600} / ${requirement.price_unit || 'quintal'}`,
          quality: 'Standard APMC Grade, Dry Grain',
          status: 'accepted',
  ...
  const handleAction = (appId, newStatus, message) => {
      ...
      >
        <motion.div
          ...
          style={{
            ...
            background: '#0a1a0d', border: '1px solid rgba(134, 239, 172, 0.2)',
        ...
        >
          {toast && (
            <div style={{
              ...
              padding: '8px 16px', borderRadius: 10, fontWeight: 800, fontSize: '0.8rem',
            }}>
              ✅ {toast}
            </div>
          )}

          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem', borderBottom: '1px solid rgba(134,239,172,0.1)', paddingBottom: '1rem' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: '0.72rem', fontWeight: 800, background: 'rgba(167,139,250,0.2)', color: '#c084fc', padding: '2px 8px', borderRadius: 6 }}>
                  {requirement.requirement_code || 'REQ-1001'}
                </span>
                <h2 style={{ fontFamily: "'Outfit', sans-serif", fontSize: '1.3rem', fontWeight: 900, color: '#fff' }}>
                  Farmer Offers for {requirement.crop_name}
                </h2>
              </div>
              <p style={{ color: 'rgba(134,239,172,0.7)', fontSize: '0.8rem', marginTop: 4 }}>
                Required: <strong>{requirement.quantity_required} {requirement.quantity_unit}</strong> • Target Price: <strong>₹{requirement.min_price} – ₹{requirement.max_price}</strong>
              </p>
            </div>
            <button type="button" onClick={onClose} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer' }}>
              <X size={20} />
            ...
            {apps.map(app => (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <h3 style={{ fontSize: '1rem', fontWeight: 800, color: '#fff' }}>{app.farmerName}</h3>
                    <p style={{ fontSize: '0.75rem', color: 'rgba(134,239,172,0.7)', display: 'flex', alignItems: 'center', gap: 4, marginTop: 2 }}>
                      <Phone size={12} /> {app.farmerPhone} • <MapPin size={12} /> {app.location}
                    </p>
                  </div>
                  <span style={{
                    padding: '4px 10px', borderRadius: 8, fontSize: '0.72rem', fontWeight: 800,
                    background: app.status === 'accepted' ? 'rgba(74,222,128,0.2)' : app.status === 'rejected' ? 'rgba(239,68,68,0.2)' : 'rgba(250,204,21,0.2)',
                    color: app.status === 'accepted' ? '#4ade80' : app.status === 'rejected' ? '#ef4444' : '#facc15',
                  }}>
                    {app.status === 'accepted' ? '✅ Accepted' : app.status === 'rejected' ? '❌ Declined' : '⏳ Pending Offer'}
                  </span>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.5rem', background: 'rgba(0,0,0,0.3)', padding: '0.6rem 0.8rem', borderRadius: '0.75rem', fontSize: '0.78rem' }}>
                  <div>
                    <span style={{ color: 'rgba(255,255,255,0.5)', display: 'block', fontSize: '0.68rem' }}>OFFERED QUANTITY</span>
                    <strong style={{ color: '#fff' }}>{app.quantity}</strong>
                  </div>
                  <div>
                    <span style={{ color: 'rgba(255,255,255,0.5)', display: 'block', fontSize: '0.68rem' }}>OFFERED PRICE</span>
                    <strong style={{ color: '#4ade80' }}>{app.price}</strong>
                  </div>
                  <div>
                    <span style={{ color: 'rgba(255,255,255,0.5)', display: 'block', fontSize: '0.68rem' }}>QUALITY GRADE</span>
                    <strong style={{ color: '#facc15' }}>{app.quality}</strong>
                  </div>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.4rem' }}>
                  <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)' }}>Submitted {app.submittedAt}</span>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    {app.status === 'pending' && (
                      <>
                        <button
                          type="button"
                          onClick={() => handleAction(app.id, 'rejected', 'Offer declined.')}
                          style={{
                        ...
                        <button
                          type="button"
                          onClick={() => handleAction(app.id, 'accepted', `Contract created for ${app.farmerName}!`)}
                          style={{
                            padding: '5px 14px', borderRadius: 8, border: 'none',
                            background: 'linear-gradient(135deg, #166534, #15803d)', color: '#86efac',
...
export default function VendorRequirementsPage() {
  ...
  return (
    <div style={{ padding: 'clamp(1rem, 3vw, 2rem)', maxWidth: 1200, margin: '0 auto', fontFamily: "'Inter', system-ui, sans-serif" }}>
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', gap: '0.5rem', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '0.75rem' }}>
                  ...
                  {req.status === 'active' && (
                    <button type="button" onClick={() => setSelectedReqForApps(req)} style={{
                      padding: '6px 14px', borderRadius: 8, border: 'none',
                      background: 'rgba(167,139,250,0.15)', color: '#a78bfa',
                      fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer',
                      display: 'flex', alignItems: 'center', gap: 4,
                    }}>
                      <Eye size={12} /> View {req.total_applications || 3} Applications
                    </button>
                  )}
      ...
      {/* Farmer Applications Modal */}
      <ApplicationsModal
        isOpen={!!selectedReqForApps}
        onClose={() => setSelectedReqForApps(null)}
        requirement={selectedReqForApps}
      />
    </div>
  );
}
