...
export default function ChatPage() {
    ...
    const isPinned = (id) => {
        ...
        if (typeof pinnedIds.has === 'function') return pinnedIds.has(id);
        if (Array.isArray(pinnedIds)) return pinnedIds.includes(id);
        return false;
    };

    const pinnedCount = pinnedIds ? (typeof pinnedIds.size === 'number' ? pinnedIds.size : (Array.isArray(pinnedIds) ? pinnedIds.length : 0)) : 0;

    const pinnedMessages = messages.filter(m => isPinned(m.id));
                ...
                <header className={`chat-header-glass px-4 md:px-8 py-3 sm:py-4 flex items-center justify-between flex-shrink-0 relative z-40 border-b backdrop-blur-xl transition-all duration-500 ${
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <motion.button 
                            ...
                            } ${pinnedCount > 0 ? (theme === 'light' ? 'border-emerald-300 text-emerald-600 bg-emerald-50' : 'control-btnActive') : ''}`}
                        >
