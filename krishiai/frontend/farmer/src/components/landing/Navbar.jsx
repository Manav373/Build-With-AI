            ...
            {menuOpen && (
              ...
              >
                <nav className="flex flex-col items-center gap-6 text-2xl text-gray-800 dark:text-white font-bold">
                  ...
                  <div className="flex flex-col items-center justify-center gap-2 mt-4 px-6 w-full">
                    {LANGUAGES.map(lang => (
                      <button
                        key={lang.code}
                        type="button"
                        onClick={() => setLanguage(lang.code)}
                        className={`text-sm px-5 py-3 rounded-full border transition-all w-48 ${language === lang.code ? 'border-[#86efac] bg-[#166534]/30 text-[#86efac]' : 'border-gray-300 dark:border-white/10 text-gray-600 dark:text-white/50'}`}
                      >
                        {lang.label}
                      </button>
                    ))}
