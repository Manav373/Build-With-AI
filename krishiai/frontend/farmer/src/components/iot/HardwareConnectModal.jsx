...
export default function HardwareConnectModal({ 
  ...
  const [bluetoothPorts, setBluetoothPorts] = useState([]);
  ...
  const refreshPorts = async (silent = false) => {
    ...
    try {
      ...
      if (data.success) {
        const realPorts = data.realPorts || data.ports || [];
        const btPorts = data.bluetoothPorts || [];
        const prevPorts = previousPortsRef.current;

        // Check if a new USB hardware device was just plugged in
        const newlyAdded = realPorts.filter(np => !prevPorts.some(op => op.path === np.path));
        if (newlyAdded.length > 0 && prevPorts.length > 0) {
          setSelectedPortPath(newlyAdded[0].path);
          addLog(`[AUTO-DETECT] 🔌 Real ESP32 / USB Hardware Plugged In: ${newlyAdded[0].path} (${newlyAdded[0].friendlyName})`);
        }

        // Auto-select first real USB port if nothing selected or current is empty
        if (!selectedPortPath && realPorts.length > 0) {
          setSelectedPortPath(realPorts[0].path);
        }

        previousPortsRef.current = realPorts;
        setAvailablePorts(realPorts);
        setBluetoothPorts(btPorts);
      }
    } catch (e) {
  ...
  const handleServerConnectSerial = async (overridePort = null) => {
    const targetPort = overridePort || selectedPortPath;
    if (!targetPort) return;
    try {
      const res = await fetch('/api/serial/connect', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ path: targetPort, baudRate })
      });
      const data = await res.json();
      if (data.success) {
        setServerSerialConnected(true);
        setSelectedPortPath(targetPort);
        addLog(`[SERVER] 🔌 Successfully connected backend to ${targetPort} at ${baudRate} baud`);
      } else {
        ...
        {activeMode === 'usb' && (
          <div>
            ...
            <div style={{ background: 'var(--bg-secondary)', padding: '1.25rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-subtle)', marginBottom: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.85rem' }}>
                <div>
                  ...
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '2px' }}>
                    Real ESP32 USB serial hardware detected on your computer:
                  </div>
                </div>
              ...
              {/* Real USB Devices List */}
              {availablePorts.length === 0 ? (
                <div style={{ background: 'rgba(234, 179, 8, 0.07)', border: '1px dashed rgba(234, 179, 8, 0.3)', borderRadius: 'var(--radius-sm)', padding: '1rem', marginBottom: '1rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--amber-400)', fontWeight: 700, fontSize: '0.85rem', marginBottom: '0.3rem' }}>
                    <AlertTriangle size={16} /> No Physical USB ESP32 Board Detected
                  </div>
                  <div style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', lineHeight: 1.45 }}>
                    Connect your ESP32 board to your PC using a USB data cable. The dashboard auto-detects it every 1.5s and will show it here immediately. You can also manually specify your port (e.g. <strong style={{ color: '#fff' }}>COM14</strong>) below.
                  </div>
                </div>
              ) : (
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          background: isSelected ? 'rgba(22, 163, 74, 0.18)' : 'var(--bg-surface)',
                          cursor: 'pointer',
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                          ...
                          <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem' }}>
                              <span className="font-mono" style={{ fontWeight: 700, fontSize: '0.88rem', color: '#ffffff' }}>{p.path}</span>
                              <span style={{ fontSize: '0.68rem', padding: '1px 6px', borderRadius: '3px', background: 'rgba(22, 163, 74, 0.25)', color: 'var(--emerald-400)', fontWeight: 600 }}>
                                {p.isEsp32 ? 'ESP32 Hardware' : 'Real USB Serial'}
                              </span>
                              {isSelected && (
                                <span style={{ fontSize: '0.65rem', padding: '1px 5px', borderRadius: '3px', background: 'var(--emerald-500)', color: '#000000', fontWeight: 700 }}>
                                  ACTIVE
                                </span>
                              )}
                            </div>
                            <div style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', marginTop: '1px' }}>
                              {p.friendlyName} {p.manufacturer && p.manufacturer !== 'Standard' ? `• ${p.manufacturer}` : ''}
                            </div>
                          </div>
                        </div>
                        {!serverSerialConnected && (
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              setSelectedPortPath(p.path);
                              handleServerConnectSerial(p.path);
                            }}
                          >
                            Connect
                          </button>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr auto', gap: '0.75rem', alignItems: 'center' }}>
                ...
                <div>
                  ...
                  >
                    <option value={115200} style={{ backgroundColor: '#090f1d', color: '#ffffff' }}>115200 Baud (KrishiAI Default)</option>
                ...
                <div style={{ paddingTop: '1.2rem' }}>
                  {!serverSerialConnected ? (
                    <button 
                      ...
                      className="btn-primary" 
                      onClick={() => handleServerConnectSerial(selectedPortPath)}
              ...
              {/* Optional Bluetooth / Virtual Ports Collapsible */}
              {bluetoothPorts.length > 0 && (
                <div style={{ marginTop: '0.85rem', paddingTop: '0.75rem', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  >
                    <Radio size={12} />
                    <span>{showBluetooth ? 'Hide' : 'Show'} {bluetoothPorts.length} Windows Bluetooth/Virtual COM links</span>
                  </button>
                      ...
                      {bluetoothPorts.map(p => (
                        <button
                          ...
                          style={{
                            ...
                            padding: '2px 6px',
                            borderRadius: '3px',
                            background: selectedPortPath === p.path ? 'rgba(56, 189, 248, 0.2)' : 'rgba(255,255,255,0.03)',
                            border: '1px solid var(--border-subtle)',
                            color: selectedPortPath === p.path ? 'var(--sky-400)' : 'var(--text-muted)',
                            cursor: 'pointer'
                          }}
                        >
                          {p.path} (BT)
