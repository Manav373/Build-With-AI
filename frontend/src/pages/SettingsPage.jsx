    ...
    <div className="flex-1 overflow-y-auto bg-[var(--page-bg)]">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-4 sm:py-8 space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
          ...
          <div className={`border rounded-2xl divide-y transition-colors ${
            theme === 'light' ? 'bg-white border-gray-100 shadow-sm divide-gray-100' : 'bg-white/[0.03] border-white/5 divide-white/5'
          }`}>
            <ToggleSetting
              icon={<MapPin size={16} />}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          ...
          <div className={`border rounded-2xl p-5 space-y-4 transition-colors ${theme === 'light' ? 'bg-white border-gray-100 shadow-sm' : 'bg-white/[0.03] border-white/5'}`}>
            <div className="flex items-center justify-between">
              ...
              <button className={`px-4 py-2 rounded-xl border text-[0.75rem] font-bold transition-all ${
                theme === 'light' ? 'bg-gray-50 border-gray-100 text-slate-500 hover:text-slate-800' : 'bg-white/[0.04] border-white/10 text-white/50 hover:text-white'
              }`}>
                Update
              </button>
            </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
          ...
          <div className={`border rounded-2xl p-5 space-y-2 transition-colors ${
            theme === 'light' ? 'bg-white border-gray-100 shadow-sm' : 'bg-white/[0.03] border-white/5'
          }`}>
            {[
...
function ProfileField({ label, value, mono }) {
  ...
  return (
    <div className={`rounded-xl p-3 border transition-colors ${
      theme === 'light' ? 'bg-gray-50 border-gray-100' : 'bg-[var(--dk)]/10 border-[var(--glass-border)]'
    }`}>
